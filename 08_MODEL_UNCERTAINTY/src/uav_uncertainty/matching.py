...
@dataclass
class TargetCluster:
    """Detections believed to represent one target across samples."""

    cluster_id: int
    observations: dict[int, Detection] = field(default_factory=dict)

    def add(self, sample_index: int, detection: Detection) -> None:
        """Add one observation, rejecting duplicate sample assignments."""

        if sample_index in self.observations:
            raise ValueError(
                f"Cluster {self.cluster_id} already has sample {sample_index}"
            )
    ...
    def reference_box(self) -> BoundingBox:
        ...
        if not self.observations:
            raise ValueError("Cannot calculate a reference for an empty cluster")
...
def match_detection_samples(
    samples: Sequence[Sequence[Detection]], iou_threshold: float = 0.50
) -> list[TargetCluster]:
    """Cluster samples using deterministic one-to-one highest-IoU matching.

    Class identity is intentionally ignored because class disagreement is a measured
    outcome. Unmatched detections form new clusters in stable geometric order.
    """
        ...
        for cluster_index, cluster in enumerate(clusters):
            ...
            for detection_index, detection in enumerate(detections):
                ...
                if overlap >= iou_threshold:
                    candidates.append(
                        (overlap, cluster_index, detection_index, _detection_key(detection))
                    )
        candidates.sort(
            key=lambda item: (-item[0], clusters[item[1]].cluster_id, item[3])
        )
