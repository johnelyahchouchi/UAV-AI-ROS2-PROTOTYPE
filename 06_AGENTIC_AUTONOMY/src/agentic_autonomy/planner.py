...
from . import PHASE1_PLANNER_VERSION
from .allocator import allocate
from .serialization import fingerprint, task_dict
from .task_generator import generate_tasks


def build_plan(scenario, policy, candidate_selector=None) -> dict:
    tasks = generate_tasks(scenario)
    assignments, unassigned, evaluations = allocate(tasks, scenario, policy, candidate_selector=candidate_selector)
    counts = {u.id: sum(a["uav_id"] == u.id for a in assignments) for u in sorted(scenario.uavs, key=lambda x: x.id)}
    plan = {"schema_version": "1.0", "planner_version": PHASE1_PLANNER_VERSION, "policy_version": policy["policy_version"],
