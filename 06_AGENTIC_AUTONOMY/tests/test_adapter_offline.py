...
def test_basic_offline_history_matches_expected_fixture_cross_platform(tmp_path):
    history = _run_basic()
    ...
    normalize_newlines = lambda value: value.replace(b"\r\n", b"\n")
    assert normalize_newlines(actual.read_bytes()) == normalize_newlines(
        expected.read_bytes()
    )
