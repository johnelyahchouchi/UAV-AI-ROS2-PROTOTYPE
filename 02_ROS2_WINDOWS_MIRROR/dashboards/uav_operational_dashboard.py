...
class UAVOperationalDashboard(Node):
    def __init__(self):
        super().__init__("uav_operational_dashboard")
        ...
        cv2.namedWindow("UAV Operational Dashboard", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("UAV Operational Dashboard", 1280, 720)

        self.get_logger().info("UAV operational dashboard started.")
    ...
    def render(self):
        ...
        if self.frame is None:
            ...
            cv2.imshow("UAV Operational Dashboard", canvas)
            cv2.waitKey(1)
            return
